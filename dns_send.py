...
# - colorama
# - datetime
# Dependent modules: hostname.py dkim_handler.py personalizer.py dynamic_attachment.py logger.py mime_type.py
...
def send_email(config, sender_email, sender_name, recipient_email, subject, body, recipient_index, total_victims):
    ...
    try:
        ...
        # Determine Reply-To header value
        reply_to = config.get("SETTINGS", "reply-to", fallback="").strip()
        if not reply_to:
            reply_to = msg["From"]
        ...
        if config.getboolean("SETTINGS", "smtp_attachment", fallback=False) and config.get("SETTINGS", "attachment_mode", fallback="normal").lower() == "dynamic":
            ...
            if attachment_path:
                attachment_extension = config.get("DYNAMIC_ATTACHMENT", "dynamic_attachment_extension", fallback="").lower()
                file_extension = os.path.splitext(attachment_path)[1][1:].lower()
                if file_extension != attachment_extension:
                    log_general(f"Attachment {attachment_path} does not match the extension in config.ini", success=False)
                else:
        ...
        # Normal Attachment Handling
            ...
            for attachment in attachments:
                attachment_extension = os.path.splitext(attachment)[1][1:].lower()
                mime_type = get_mime_type(attachment_extension)
                try:
